...
//       nss.println("ricevuto coordinator address");
   ...
   //       nss.println("device presente in impianto");
      ...
      if(rx.getData(0) == 'V'){
        //nss.println("Richiesta Log file");
        sendLogFile();
  ...
  if (Serial1.available()){
      if (Serial1.peek() == 'A'){
        getRtcTime();
        //nss.println("---- inizio ciclo login");
        //int datalength = Serial3.available();
        byte data2[50];
        int i = 0;
        while (Serial1.available()){
          data2[i] = Serial1.read();
          i++;
          delay(25);
        }
        byte xbeeSender[8] = {0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x1};     
        doLoginLogout(xbeeSender, data2, i, true);
        //nss.println("---- fine ciclo login");
      }      
      if (Serial1.peek() == 'B'){
        getRtcTime();
        ...
        byte xbeeSender[8] = {0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x1};     
        doLoginLogout(xbeeSender, data2, i, false);
        //nss.println("---- fine ciclo logout");
      }
      if (Serial1.peek() == 'X'){
        getRtcTime();
        ...
        if (isInImpianto(xbeeSender)){
          Serial1.read();
          delay(25);          
          forwardException((int)Serial1.read());
        }
      }
  ...
  if (Serial2.available()){
      if (Serial2.peek() == 'A'){
        ...
        byte xbeeSender[8] = {0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x2};     
        doLoginLogout(xbeeSender, data2, i, true);
        //nss.println("---- fine ciclo login");
      }      
      if (Serial2.peek() == 'B'){
      ...
      if (Serial2.peek() == 'X'){
        ...
        if (isInImpianto(xbeeSender)){
          Serial2.read();
          delay(25);          
          forwardException((int)Serial2.read());
        }
      }
  ...
  if (Serial3.available()){
      if (Serial3.peek() == 'A'){
        ...
        byte xbeeSender[8] = {0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x3};     
        doLoginLogout(xbeeSender, data2, i, true);
        //nss.println("---- fine ciclo login");
      }      
      if (Serial3.peek() == 'B'){
      ...
      if (Serial3.peek() == 'X'){
        ...
        if (isInImpianto(xbeeSender)){
          Serial3.read();          
          delay(25);
          forwardException((int)Serial3.read());
        }
      }
...
void setDataTime(byte data[8]){
    DS1307.setDate(data[1], data[2], data[3], data[7], data[4], data[5], data[6]);
}
...
void askCoordinatorAddress(){
    ...
    zbTx = ZBTxRequest(addr64, payload, 1);
    //xbee.send(zbTx);
    sendX();    
}
...
void sendLogFile(){
      ...
      while (getLogLine(i, result)) {
        ...
        zbTx = ZBTxRequest(addr64, payload, 64);
        //xbee.send(zbTx);
        sendX();
        //nss.println("Invio Riga log");
      }
      //delay(25);
      uint8_t payloadEnd[1];
      payloadEnd[0] = 'Z';
      zbTx = ZBTxRequest(addr64, payloadEnd, 1);
      //xbee.send(zbTx);
      sendX();      
      //nss.println("Fine log");
